...
export const TemplateCatalogScreen = ({ locale, onBack, onOpenBasicLayout, onOpenTemplate, withTopSafeArea = false }: Readonly<{
  ...
  <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <BasicLayoutSection locale={locale} onOpenBasicLayout={onOpenBasicLayout} />
    <Section title={t(locale, 'templateCatalog.shapedLayouts')}>
      {shapedLayouts.map((layout) => <BasicLayoutCard key={layout.id} layout={layout} locale={locale} onPress={() => onOpenBasicLayout(layout)} />)}
    </Section>
    <Section title={t(locale, 'templateCatalog.designedTemplates')}>
      {localTemplateCatalog.map((template) => <DesignedTemplateCard key={template.id} template={template} onPress={() => onOpenTemplate(template)} />)}
    </Section>
  </ScrollView>
  </>;
...
const BasicLayoutCard = ({ compact = false, layout, locale, onPress }: Readonly<{ compact?: boolean; layout: BasicLayout; locale: ProductLocale; onPress: () => void }>) => {
  ...
  <View style={[styles.layoutPreview, compact && styles.compactLayoutPreview]}><LayoutDiagram layout={layout} /></View>
...
const DesignedTemplateCard = ({ onPress, template }: Readonly<{ onPress: () => void; template: TemplateDefinition }>) => {
  ...
  return <Pressable accessibilityLabel={`Use ${template.name} template`} accessibilityRole="button" onPress={onPress} style={styles.card}>
    ...
    <View pointerEvents="none" style={styles.titleScrim} />
    <Text numberOfLines={1} style={styles.designedTitle}>{template.name}</Text>
  </Pressable>;
};

const LayoutDiagram = ({ layout }: Readonly<{ layout: BasicLayout }>) => {
  ...
  return <View onLayout={(event) => {
    ...
    {previewSize.width > 0 && template.photoSlots.map((slot) => {
      ...
      }]}>{glyph !== null && <Text style={styles.diagramShapeGlyph}>{glyph}</Text>}</View>;
    })}
  </View>;
};
...
const styles = StyleSheet.create({
  ...
  titleScrim: { backgroundColor: 'rgba(0,0,0,0.24)', bottom: 0, height: 52, left: 0, position: 'absolute', right: 0 },
