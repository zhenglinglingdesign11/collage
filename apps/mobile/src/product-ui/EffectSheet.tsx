...
export const EffectSheet = ({ bottomInset, effects, layer, locale, onCancel, onCommit, onPreview }: Readonly<{
  ...
  const patchAdjustment = (params: Readonly<Record<string, number | CenterFrameId>>) => {
    if (!adjusting) return;
...
const effectLabel = (locale: ProductLocale, type: string, fallback: string): string => {
  const keys: Record<string, ProductCopyKey> = { 'light.shadow': 'editor.effects.shadow', 'edge.outline': 'editor.effects.outline', 'paper.torn-edge': 'editor.effects.torn', 'material.grain': 'editor.effects.grain', 'shape.round-corners': 'editor.effects.corners', 'attachment.tape': 'editor.effects.tape', 'paper.float': 'editor.effects.float', 'frame.lace-center': 'editor.effects.lace', 'frame.foil-center': 'editor.effects.foil' };
