...
- `去白边强度`：浏览器生成预览和导出 PNG 时使用，只在切片 mask 外缘附近收掉接近局部背景的浅色低纹理像素。

如果边缘仍缺失，先把 `FastSAM 补边` 调到 `4` 或 `5` 并重新分割；如果还缺，再降低 `精修容差`。如果白边过多，先调高 `去白边强度`，再调高 `精修容差` 或关闭 `局部精修边缘` 对比原始 FastSAM 结果。

## 参数

- `背景模式`：透明 PNG 用 `透明通道`；JPG 或纯色背景图用 `四角背景色`；扫描/生成的白底素材纸用 `白底素材纸`；不确定时用 `自动`。
...
- `去白边强度`：切片导出前的边缘收白强度，`0` 表示关闭，只影响 FastSAM 结果。
